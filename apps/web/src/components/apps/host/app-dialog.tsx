// apps/web/src/components/apps/host/app-dialog.tsx
'use client'
...
import { useDialog } from '~/components/apps/runtime/hooks/use-dialog'
...
export function AppDialog() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = store.events.dialogUpdated.addListener(({ dialogId }) => {
      if (dialogId === dialog.id) {
        console.log('[AppDialog] Dialog updated, re-rendering:', dialogId)
        forceUpdate()
      }
    })
   ...
   */
  const handleClose = () => {
    console.log('[AppDialog] Closing dialog:', dialog.id)
    ...
    if (messageClient) {
      console.log('[AppDialog] Notifying iframe: dialog-close-requested')
      messageClient.sendMessage('dialog-close-requested', { id: dialog.id })
    } else {
      console.warn('[AppDialog] MessageClient not found, dialog closed in store only')
    }
  }
