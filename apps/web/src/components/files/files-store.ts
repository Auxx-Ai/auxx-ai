 ...
 */
export function isUploadingItem(item: FileItem): item is FileItem & Required<Pick<FileItem, 'status'>> {
...
export interface FileSystemStore {
  ...
  setSorting: (sortBy: 'name' | 'size' | 'type' | 'created' | 'modified', sortOrder: 'asc' | 'desc') => void
...
export const useFileSystemStore = create<FileSystemStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    // Computed properties (getters)
    get allFileIds() {
      return Array.from(this.itemsById.keys()).filter(id => 
        this.itemsById.get(id)?.type === 'file'
      )
    },
    
    get allFolderIds() {
      return Array.from(this.itemsById.keys()).filter(id => 
        this.itemsById.get(id)?.type === 'folder'
      )
    },
    ...
    setFileSystemData: (items) => {
      ...
      items.forEach(item => {
        ...
        if (item.hierarchy) {
          searchTerms.push(
            ...item.hierarchy.fullPath.split('/').filter(Boolean).map(s => s.toLowerCase()),
            item.hierarchy.folderName.toLowerCase()
          )
        } else {
          // Fallback for items without hierarchy
          searchTerms.push(...item.path.split('/').filter(Boolean).map(s => s.toLowerCase()))
        }
      ...
      set(state => {
        ...
        itemIds.forEach(itemId => {
          ...
          if (item.type === 'file') {
            ...
            newFilesByParent.set(parentId, siblings.filter(id => id !== itemId))
          } else {
            const siblings = newFoldersByParent.get(parentId) || []
            newFoldersByParent.set(parentId, siblings.filter(id => id !== itemId))
          }
