...
vi.mock('../attachment-thumbnail', () => ({
  AttachmentThumbnail: vi.fn(({ fallback }) => fallback || <div data-testid="attachment-thumbnail">Attachment Thumbnail</div>)
}))
  ...
  describe('download functionality', () => {
    test('opens download URL on click', () => {
      ...
      expect(mockWindowOpen).toHaveBeenCalledWith(
        '/api/attachments/att_123/download',
        '_blank'
      )
    })
    ...
    test('handles various image types correctly', () => {
      ...
      imageTypes.forEach(({ mimeType, shouldPreview }) => {
        const { rerender } = render(
          <AttachmentDisplay 
            attachment={{ ...mockAttachment, mimeType }} 
          />
        )
