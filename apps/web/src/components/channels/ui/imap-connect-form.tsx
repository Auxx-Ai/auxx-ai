// apps/web/src/components/channels/ui/imap-connect-form.tsx
'use client'
...
interface ImapConnectFormProps {
  /** Inbox-destination controller from the detail step — resolved on submit to link the channel. */
  inbox: InboxDestinationController
  /** Fired after a successful connect (gallery closes + navigates). */
  onSuccess: () => void
}

/**
 * Shell-free IMAP/SMTP/LDAP connect form embedded in the channel gallery's detail step.
 * The inbox picker lives above it (rendered by the detail step); this form resolves that
 * choice on submit and links the channel at connect time (inbox-first).
 */
export default function ImapConnectForm({ inbox, onSuccess }: ImapConnectFormProps) {
  ...
  const testConnection = api.channel.testImapConnection.useMutation({
    onSuccess: (data) => {
      setTestResults({ imap: data.imap, smtp: data.smtp, ldap: data.ldap })
    },
    onError: (error) => {
  ...
  const onSubmit = async (data: ImapFormValues) => {
    try {
      // Resolve the destination inbox (creating it if the user chose "new"), then connect+link.
      const inboxId = await inbox.resolve()
      await connectImap.mutateAsync({ ...data, inboxId })
      onSuccess()
    } catch (error) {
      toastError({
  ...
  const submitting = connectImap.isPending || inbox.creating
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
        ...
        <div className='flex justify-end gap-2'>
          ...
          <Button
            ...
            disabled={!inbox.isValid || submitting}>
            Connect
          </Button>
        </div>
      </form>
