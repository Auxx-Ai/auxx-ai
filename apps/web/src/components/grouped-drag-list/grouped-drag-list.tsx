// apps/web/src/components/grouped-drag-list/grouped-drag-list.tsx
'use client'
...
import { GroupedDropZone, GroupedInsertLine } from './drop-zone'
import { type GroupedSection, groupItemOrder } from './group-sections'

/**
 * The minimum a group must be for this list to place, collapse and drag it.
 *
 * Everything else a group carries (a label, an icon, a colour) is the caller's
 * business and reaches `renderGroupHeader` untouched through `TGroup`. The
 * persisted field-view `FieldGroup` names its members `fieldIds`, so the field
 * panel adapts at its own boundary — see `components/fields/ui/field-group-list.tsx`.
 */
export interface GroupedDragListGroup {
  id: string
  /** The items this group claims. They render CONTIGUOUSLY, at the position of the first one. */
  itemIds: string[]
  /**
   * Where an EMPTY group renders: immediately before this item. Read only while
   * the group has no surviving member, ignored once it has one.
   */
  anchorItemId?: string
  /** The persisted default collapse state; a viewer's own toggle layers over it. */
  collapsed?: boolean
}

/** What `renderRow` is told about the slot it is rendering into. */
export interface GroupedDragListRowContext {
...
/** What `renderGroupHeader` is told about the group it is rendering. */
export interface GroupedDragListHeaderContext {
  /** Whether the group's members are hidden right now. */
  collapsed: boolean
  /** Number of member rows this group would render when expanded. */
  memberCount: number
  /** Toggle this viewer's local collapse override. A no-op in edit mode. */
  onToggleCollapsed: () => void
  isEditMode: boolean
  /**
   * Rendered inside the drag ghost. A preview is a picture of what is in hand,
   * not a live row: it must render no rename input, no action buttons and no
   * autofocus.
   */
  preview: boolean
}

export interface GroupedDragListProps<TRow, TGroup extends GroupedDragListGroup> {
  /** Rows in flat display order, already filtered to what should render. */
  rows: TRow[]
  /**
   * The ITEM id of a row — the id the groups' `itemIds` store, and every dnd id.
   * A "row" is the rendered representation of an item; the two are separate only
   * so a caller can key React by something else.
   */
  ...
  /**
   * Drop handler for an ITEM drag. `edge` names which side of the target the
   * item lands on, read from the pre-drag positions so the drop agrees with
   * the insert line.
   */
  onItemDragEnd: (event: DragEndEvent, edge?: 'before' | 'after') => void
  /** Place an item immediately before/after a group's block, belonging to NO group. */
  onPlaceItemBesideGroup?: (itemId: string, groupId: string, side: 'before' | 'after') => void
  /** Move a whole group block. `overId` is a bare group id when `overIsGroup`, else an item id. */
  onMoveGroup?: (groupId: string, overId: string, overIsGroup: boolean) => void
  /** Class applied to a GROUPED row's wrapper — surfaces indent differently. */
   ...
   * through the header's `data-slot`s — see the record dialog.
   */
  ...
  renderRow: (row: TRow, ctx: GroupedDragListRowContext) => ReactNode
  /**
   * The group header, supplied by the caller so a surface can render whatever
   * shape it wants (a field-panel group row, a `TreeRow` for a layout tab).
   *
   * The header is also the group's drag SOURCE and the drop target that makes an
   * item join the group, so an edit-mode header MUST register itself as a
   * sortable under `groupDropId(group.id)` — one `useSortable`, not a draggable
   * plus a separate droppable, since `useSortable` already registers a droppable
   * under the same id.
   */
  renderGroupHeader: (group: TGroup, ctx: GroupedDragListHeaderContext) => ReactNode
}

/**
 * A grouped list and its whole drag model: reorder within a group, move an item
 * BETWEEN groups, move a whole group block, and land an item beside a block in
 * no group at all. Generic over its item type and its group type.
 *
 ...
 * whole-group highlight. This is that model (see `drop-targets.ts` for the id
 * keyspace).
 *
 ...
 * cursor.
 */
export function GroupedDragList<TRow, TGroup extends GroupedDragListGroup>({
  rows,
  rowId,
  rowKey,
  groups,
  ...
  renderRow,
  renderGroupHeader,
}: GroupedDragListProps<TRow, TGroup>) {
   ...
   *
   * A group's `collapsed` flag is part of the org's shared default view — one
   * config row every member of the org reads. Persisting a chevron click would
   * be a server write per click that collapses the group for *everyone*, which
   * is not what "I want this section out of my way right now" means. So collapse
   * is a per-mount override layered on the persisted default.
   *
   ...
   * the field-view draft hook's `setGroupCollapsed` remain, since a "starts
   * collapsed" option is the natural place to wire it back up.
   */
   ...
   * dragging items between groups and groups past each other — and a hidden
  ...
  // and hidden items can never leave a group claiming an item the list doesn't
  // show. Empty groups only exist in edit mode, where they are drop targets;
  // read mode drops them entirely.
  const sections = groupItemOrder<TGroup>({
    itemOrder: rows.map(rowId),
    groups,
    itemIdsOf: (group) => group.itemIds,
    anchorItemIdOf: (group) => group.anchorItemId,
    includeEmptyGroups: isEditMode,
  })

  // The flat visible item sequence: a collapsed group contributes NO rows, so
  ...
  const visibleRowIds = sections.flatMap((section) =>
    section.group && isGroupCollapsed(section.group) ? [] : section.itemIds
  )
  ...
  const [dropTarget, setDropTarget] = useState<GroupedDropTarget | null>(null)

  /** The group being dragged, or null when an item row is in hand. */
  const activeGroupId = activeDragId === null ? null : parseGroupDropId(activeDragId)

  const memberIdsOfGroup = (groupId: string): string[] =>
    sections.find((section) => section.group?.id === groupId)?.itemIds ?? []

  /**
   * What the drag ghost renders: the whole block for a group drag, one row for
   * an item drag. Null when nothing is in hand, which is what keeps the overlay
  ...
  // Narrowed here rather than cast at the render site: the lookup above matched
  // on `section.group?.id`, so a hit always carries a group.
  const activeGroup = activeGroupSection?.group ?? null
  const activeRow =
    activeDragId !== null && activeGroupId === null ? rowById.get(activeDragId) : undefined

  // Positions are compared in the RENDERED order (which is the flat item order,
  // group-contiguous) so the insert line can be drawn on the edge the drop
  // actually lands on — see `deriveDropFeedback`.
  const renderedOrder = sections.flatMap((section) => section.itemIds)
  const orderIndexById = new Map(renderedOrder.map((itemId, index) => [itemId, index]))
  const groupIdByItemId = new Map<string, string>()
  for (const section of sections) {
    if (!section.group) continue
    for (const itemId of section.itemIds) groupIdByItemId.set(itemId, section.group.id)
  }

  /** Where the dragged thing sits today: an item's slot, or a block's anchor. */
  ...
  /**
   * The caller's reorder and block-move are both direction-sensitive: dragging
   ...
   *
   * - `blocked: false` (blue) — an ITEM about to join the group. It really will
   *   be a member.
   ...
   * item outside every group, so there is no group to be inside of.
   */
  const deriveDropFeedback = (): {
    ...
    // A GROUP drag always snaps to a whole block boundary (a block move never
    // splits another group), so an item target resolves to that item's group
    // when it has one.
    if (activeGroupId !== null) {
      const overItemId =
        dropTarget.kind === 'item' || dropTarget.kind === 'item-before' ? dropTarget.itemId : null
      const referenceGroupId =
        overItemId !== null
          ? (groupIdByItemId.get(overItemId) ?? null)
          : dropTarget.kind === 'item' || dropTarget.kind === 'item-before'
            ? null
            : dropTarget.groupId
      if (referenceGroupId === null) {
        const itemId = overItemId
        if (itemId === null) return { indicator: null, highlight: null }
        const side = edgeFor(orderIndexById.get(itemId) ?? -1)
        return {
          indicator: { kind: 'row', rowId: itemId, side, inset: false },
          highlight: null,
        }
      }
    ...
    switch (dropTarget.kind) {
      case 'item': {
        const side = edgeFor(orderIndexById.get(dropTarget.itemId) ?? -1)
        const targetGroupId = groupIdByItemId.get(dropTarget.itemId) ?? null
        return {
          indicator: { kind: 'row', rowId: dropTarget.itemId, side, inset: false },
      ...
      case 'item-before': {
        ...
        const targetGroupId = groupIdByItemId.get(dropTarget.itemId) ?? null
        return {
          indicator: { kind: 'row', rowId: dropTarget.itemId, side: 'top', inset: false },
          highlight: targetGroupId === null ? null : { groupId: targetGroupId, blocked: false },
        }
      }
      case 'group-into': {
        // An item dropped on the header lands at the HEAD of the block, so the
  ...
  const sectionsRef = useRef<GroupedSection<TGroup>[]>(sections)
   ...
   *
   * - an ITEM may not target its own row (`x-before` and the bare `x` both
   ...
   *   in the flat order (its end-of-list slot is a render convention), so a
   *   block move has no honest insertion point and refuses every one of its
   *   three droppables. Letting it become `over` drew a highlight and an insert
   *   line for a drop that then did nothing. An ITEM drag still targets it —
   *   that is the only way a new group ever gets its first member.
   *
  ...
  const collisionDetection = useCallback<CollisionDetection>((args) => {
    ...
    if (draggedGroupId === null) {
      /** The item in hand is already the last thing in that group's block. */
      const isLastMemberOf = (groupId: string): boolean => {
        const memberIds = sectionsRef.current.find((s) => s.group?.id === groupId)?.itemIds ?? []
      ...
      const usable = collisions.filter((collision) => {
        ...
        if (target.kind === 'item' || target.kind === 'item-before')
          return target.itemId !== draggedId
        // "Inside, last slot" is where this item already IS — the drop would be
    ...
    const ownMemberIds = new Set(
      sectionsRef.current.find((section) => section.group?.id === draggedGroupId)?.itemIds ?? []
    )
    // An EMPTY group is a legitimate target here, unlike for an item drag where
    // "inside" and "beside" genuinely differ. Every group-addressing target
    // collapses to one `onMoveGroup` call (see `routeDragEnd`), so "before an
    // empty group" and "after an empty group" are both just an order change and
    // there is nothing ambiguous to resolve. Excluding them made a group with
    // no members unreachable as a neighbour, which stranded any tab that holds
    // no sections: a freshly created one, and every base tab.
    return collisions.filter((collision) => {
      const target = resolveDropTarget(String(collision.id))
      if (target.kind === 'item' || target.kind === 'item-before')
        return !ownMemberIds.has(target.itemId)
  ...
  const dropTargetKey = (target: GroupedDropTarget): string =>
    target.kind === 'item' || target.kind === 'item-before'
   ...
   *
   * A group header and an item row are the same kind of thing to dnd-kit but
   * different operations here: dragging a group repositions its whole member
   * block (order only), while dragging an item can also change which group it
   * belongs to. Running the item path for a group drag would reassign membership
   * from wherever the header happened to land, which is why the two never share
   * a code path.
   *
   * Every branch translates the suffixed droppable id back to the bare item or
   * group id the caller's handlers already expect; neither handler is modified.
   */
  const routeDragEnd = (event: DragEndEvent) => {
    ...
    // same call: the caller's block move owns the snap-to-boundary and direction
    // rules, so "above" vs "below" is its decision, not the droppable's.
    if (draggedGroupId !== null) {
      if (target.kind === 'item' || target.kind === 'item-before') {
        onMoveGroup?.(draggedGroupId, target.itemId, false)
        return
      }
    ...
    switch (target.kind) {
      case 'item-before':
        ...
        if (target.itemId === activeId) return
        onItemDragEnd(aimedAt(event, target.itemId), 'before')
        return
      case 'item': {
        if (target.itemId === activeId) return
        ...
        // `onItemDragEnd` may relocate the item (joining a group appends it to
        // the block's tail) before it reorders, which would flip the direction.
        const side = edgeFor(orderIndexById.get(target.itemId) ?? -1)
        onItemDragEnd(aimedAt(event, target.itemId), side === 'bottom' ? 'after' : 'before')
        return
      }
      case 'group-into':
        onItemDragEnd(aimedAt(event, groupDropId(target.groupId)))
        return
      case 'group-end': {
        // Aim at the last member with an explicit `after`. `onItemDragEnd`
        // reads membership from where the item lands, so targeting a member
        ...
        onItemDragEnd(aimedAt(event, lastMemberId), 'after')
        return
      }
      case 'group-before':
        onPlaceItemBesideGroup?.(activeId, target.groupId, 'before')
        return
      case 'group-after':
        onPlaceItemBesideGroup?.(activeId, target.groupId, 'after')
        return
    }
  }

  /** A section's member rows, in the section's own order, ghosts skipped. */
  const memberRowsOf = (itemIds: string[]): TRow[] =>
   ...
   */
  const renderHeader = (group: TGroup, memberCount: number, preview = false) =>
  ...
  const renderMemberRow = (
    ...
    return (
      ...
      //
      // `dimmed` fades a member of the group currently in hand. The header
      // already fades itself via `isDragging`, but a member row has no idea its
      // group is being dragged — without this only the header dimmed and the
      // block did not read as lifted. Safe now that nothing displaces: there
      ...
      <div
        ...
        {isEditMode && !options.preview && (
          <>
            <GroupedDropZone id={itemBeforeDropId(id)} edge='top' />
            {lineForRow(id, 'top') && <GroupedInsertLine side='top' />}
            {lineForRow(id, 'bottom') && <GroupedInsertLine side='bottom' />}
          </>
        )}
        {renderRow(row, {
  ...
  const renderSections = () =>
    sections.map((section, sectionIndex) => {
      ...
      const memberRows = collapsed ? [] : memberRowsOf(section.itemIds)
      ...
      return (
        <div
          ...
          {isEditMode && (
            <>
              <GroupedDropZone id={groupBeforeDropId(group.id)} edge='top' />
              ...
              {memberRows.length > 0 && (
                <GroupedDropZone id={groupEndDropId(group.id)} edge='inner-bottom' />
              )}
              <GroupedDropZone id={groupAfterDropId(group.id)} edge='bottom' />
              {highlight?.groupId === group.id && (
                <div className='pointer-events-none absolute inset-0 z-10 rounded-md border border-primary-200 border-dashed' />
              )}
              {lineForGroup(group.id, 'top') && <GroupedInsertLine side='top' />}
              {lineForGroup(group.id, 'bottom') && (
                <GroupedInsertLine side='bottom' inset={indicator?.inset ?? false} />
              )}
            </>
          )}
          {renderHeader(group, section.itemIds.length)}
          {rowElements}
        </div>
      )
    })
      ...
      {typeof document !== 'undefined' &&
        createPortal(
          <DragOverlay
            ...
            {activeGroupSection && activeGroup ? (
              ...
              <div
                ...
                {renderHeader(activeGroup, activeGroupSection.itemIds.length, true)}
                {memberRowsOf(activeGroupSection.itemIds).map((row) =>
              ...
              <div className='rounded-md bg-background/90 opacity-90 shadow-lg ring-1 ring-border'>
                {renderMemberRow(activeRow, {
                  grouped: groupIdByItemId.has(rowId(activeRow)),
                  preview: true,
                })}
              </div>
            ) : null}
