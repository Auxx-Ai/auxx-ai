...
//
// Draft-preview falls out for free: the query key includes `configuration`, so
// editing settings in the config panel re-fetches with the draft config.
// `keepPreviousData` avoids skeleton flashes while tweaking; `isChartConfigured`
// gates the request so unconfigured shells never hit the server.

import { type ChartWidgetConfig, isChartConfigured } from '@auxx/lib/dashboards/client'
