 ...
 * Renders a dialog for creating a new support order for a contact
 */
export default function CreateOrderDialog({
  contactId,
  onSuccess,
  children,
}: CreateOrderDialogProps & { children?: React.ReactNode }) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-h-screen max-w-3xl overflow-y-scroll">
        <DialogHeader>
          ...
          <DialogDescription>Fill out the form below to create a new order.</DialogDescription>
        </DialogHeader>
        <TicketForm
          contactId={contactId}
          onSuccess={() => {
