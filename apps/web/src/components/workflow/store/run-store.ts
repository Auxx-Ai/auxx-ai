...
// `subscribeWithSelector` matches every other store in this folder and is what
// the `subscribe(selector, listener)` call sites expect — without it zustand
// treats the SELECTOR as the listener and silently drops the real handler.
export const useRunStore = create<RunState & RunActions>()(
  subscribeWithSelector(
    immer((set, get) => ({
      ...
      // },
      // Helper methods for event hooks
      setNodeExecutions: (nodeExecutions: Map<string, NodeExecutionState>) => {
        set((state) => {
