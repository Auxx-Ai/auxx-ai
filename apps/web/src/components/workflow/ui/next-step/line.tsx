// apps/web/src/components/workflow/ui/next-step/line.tsx

import { memo } from 'react'

type LineProps = { list: number[] }
const Line = ({ list }: LineProps) => {
  ...
  const svgHeight = (processedList[processedListLength - 1] ?? 0) + (processedListLength - 1) * 8

  return (
    <svg className='w-6 shrink-0' style={{ height: svgHeight }}>
      {processedList.map((item, index) => {
        const prevItem = (index > 0 ? processedList[index - 1] : 0) ?? 0
