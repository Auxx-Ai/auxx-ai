// apps/web/src/components/custom-fields/ui/custom-field-form.tsx
'use client'
...
/** Context passed to the host's optional header slot. */
export interface CustomFieldFormHeaderContext {
  title: string
  description: string
}

/** Props for CustomFieldForm */
export interface CustomFieldFormProps {
  /** Gates the form-reset effect (only resets while the host surface is open). */
  open: boolean
  /** ResourceFieldId for edit mode, null/undefined for create mode */
  ...
  /** Close after a successful save (unless "Create more" is on). */
  onClose: () => void
  /** Cancel / back — routed through the host's unsaved-changes guard. */
  onRequestClose: () => void
  /** Report dirty state up so the host guard can intercept a close. */
  onDirtyChange?: (isDirty: boolean) => void
  /** Host supplies the title chrome (modal → DialogHeader; palette → breadcrumb, omitted). */
  header?: (ctx: CustomFieldFormHeaderContext) => ReactNode
  /** Fires when the selected type wants a wider shell (RELATIONSHIP). The modal
   *  toggles DialogContent size; the palette toggles its DialogNavPage size. */
  onWideChange?: (wide: boolean) => void
}

/**
 * Shell-less core for creating/editing custom fields. Owns all field state, the
 * type-specific editors, and submit — but no `Dialog` chrome and no
 * unsaved-changes guard (those belong to the host). Mounted by
 * {@link CustomFieldDialog} (modal) and by the command palette's create-field
 * page. The host seam is the `header` slot plus the `onClose` /
 * `onRequestClose` / `onDirtyChange` / `onWideChange` callbacks.
 */
export function CustomFieldForm({
  ...
  }, [open, resourceFieldId, editingField, onRequestClose])
  ...
  // Track whether to keep the form open after creating
  ...
  // Report dirty state up so the host's unsaved-changes guard can intercept.
  useEffect(() => {
    onDirtyChange?.(isDirty)
  }, [isDirty, onDirtyChange])

  // Reset form when the host opens or editing field changes
  // biome-ignore lint/correctness/useExhaustiveDependencies: resourceFieldId is stable when the host opens
  useEffect(() => {
    if (open) {
  ...
  // Tell the host whether the selected type wants a wider shell (RELATIONSHIP).
  useEffect(() => {
    onWideChange?.(selectedType === FieldType.RELATIONSHIP)
  }, [selectedType, onWideChange])
    ...
    <>
      {header?.({
        ...
        description: isEditing
          ...
          : 'Configure your new custom field.',
      })}
      <Form {...form}>
