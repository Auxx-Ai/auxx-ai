...
const withServerActionInstrumentation = async (
  name: string,
  options: { recordResponse?: boolean },
  callback: () => Promise<ServerActionResponse<any, any>>
): Promise<ServerActionResponse<any, any>> => {
  // return await callback
  return async () => {
    console.log('lala', name, options)
    return callback()
  }
  // const transaction = startTransaction({ name, op: 'action' })
}
/**
...
>(
  ...
  return async (
    ...args: Args
  ): Promise<ServerActionResponse<EnsureObject<Result> & { success: boolean }, Err>> => {
    try {
      const result = await withServerActionInstrumentation(
        ...
        async () => {
          try {
            ...
            if (!res) {
              return { success: true } as EnsureObject<Result> & { success: boolean }
            }

            if ('error' in res) return res

            return { success: true, ...res } as unknown as EnsureObject<Result> & { success: true }
          } catch (error) {
            ...
            if (apiError) {
              ...
              return { error: apiError.message, success: false } as unknown as ActionError<Err>
            }
      ...
      return { error: 'An error occurred', success: false } as ActionError<Err & { success: false }>
    }
  }
}
