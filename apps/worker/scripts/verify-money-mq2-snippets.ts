 ...
 *   4. `updateSnippet` / `deleteSnippet` / `setSnippetSharing` all throw Forbidden.
 *
...
async function main() {
  ...
  check('sharingType = PRIVATE', snippet1.sharingType === 'PRIVATE')
  check(
  ...
  const listResult = await listSnippetsForUser(database, organizationId, userId, {
    includeShared: true,
  })
  ...
  const updateResult = await updateSnippet(database, organizationId, userId, snippet1.id, {
    title: 'hacked',
  })
  check(
  ...
  const shareResult = await setSnippetSharing(
    database,
    organizationId,
    userId,
    snippet1.id,
  ...
  check(
    'setSnippetSharing returns err (Not-creator or Forbidden — createdById is systemUser)',
    shareResult.isErr(),
    shareResult.isErr() ? shareResult.error.message : shareResult.value
  )

  const deleteResult = await deleteSnippet(database, organizationId, userId, snippet1.id)
  check(
