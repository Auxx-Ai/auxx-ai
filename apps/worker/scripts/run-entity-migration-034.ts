// apps/worker/scripts/run-entity-migration-034.ts
/**
 * One-off runner for entity migration 034 (add the hidden `quote.pdfAsset` field,
 * money MQ2 build spec §C.1) across all orgs. Runs ONLY 034 — deliberately not
...
async function main() {
  const migration = ALL_ENTITY_MIGRATIONS.find((m) => m.id === '034-quote-pdf-asset-field')
  if (!migration) throw new Error('Migration 034 not found in registry')
