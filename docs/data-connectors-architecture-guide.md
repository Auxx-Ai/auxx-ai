...
| Table | File | Role |
|-------|------|------|
| `DataConnector` | `data-connector.ts` | The connector definition: `type`, `config` (jsonb — generic-rest `endpoint` + filters + `historyStartDate` + `webhookTrigger` SIGNAL), bound `credentialId`, `syncBehavior` + `scheduleConfig`, `status`, `itemCount`, `lastSyncedAt`, `error`. One per type per org (v1). |
| `DataConnectorStream` | `data-connector-stream.ts` | One fetch = one source schema. Holds `streamKey` (nullable — a draft is created blank and named inline), `requestConfig` (jsonb), `sourceSchema` + `schemaSource` (`catalog`/`inferred`/`manual`), `syncMode` (`snapshot`/`incremental`; an app stream's is derived from its catalog `query` — `since` ⇒ incremental — never declared), `recordFilter` (jsonb condition groups over source paths, evaluated per raw record by `record-filter.ts` after the fetch and never sent to the app; an app seeds it from the stream's `recordFilter` declaration and a redeploy updates it like `syncMode`), and **`state`** (jsonb `ConnectorStreamState` — the durable per-stream sync state: phase, backfill cursor, watermark, recordsSeen). |
...
| `DataConnectorItem` | `data-connector-item.ts` | Durable upstream↔instance binding, **per mapping**. Unique `(dataConnectorId, mappingId, externalId)`. Carries content hash + cursor state + `lastSeenRunId` (drives orphan reconciliation) off the entity row, and `parentExternalId` for children reconciled per parent. |
| `DataConnectorRun` | `data-connector-run.ts` | Health/run ledger, **one per sync** (the whole slice chain folds into it). Carries `trigger`, `mode`, `status` (`running`/`completed`/`partial`/`failed`), engine-managed `phase` (`backfill`/`steady`), per-category counts (`created`/`updated`/`skipped`/`archived`/`deleted`/`failed`/`relationshipWarnings`), `pagesProcessed`, `rateLimitWaitMs`, a `progress` jsonb snapshot for the live status line, `query` (a re-import's `ConnectorQuery`; null on a sync), `startedAt`/`heartbeatAt`/`finishedAt`. |
...
- `backfillStartedAt`: when the current backfill chain began.
- `watermark`: steady-phase delta marker. Generic REST: the max `watermarkField` seen, folded with `maxWatermark`. App stream: the app's opaque `since`, JSON-encoded, **replaced** (never compared) on the terminal checkpoint of each run.
- `recordsSeen`: running total for the progress UI.
- `cursor`: legacy single-shot generic-rest cursor. `phase` is the only "backfill finished" signal.

The history floor is not stream state. `config.historyStartDate` is a fixed day, so `startConnectorSync` computes it once per chain (`historyFloor`) and pins it on `run.chainSnapshot.floor`; every slice of the run sends the same `period.from`.
...
interface ConnectorCheckpoint {
  ...
  watermark?: string        // generic REST: max watermark observed through this page
  since?: string            // app stream, last page only: the encoded `since` (replaces the watermark)
}
...
`fetch` receives `ConnectorFetchArgs`: `{ streamKey, query: ConnectorQuery, mode, state, credential, config, requestConfig?, triggerContext?, rateLimitOverride?, onPageMeta? }`. The sliced `SyncSource` sets `rateLimitOverride: { maxRetries: 0 }` so the throttle returns immediately (slicing owns the backoff, not the transport). `onPageMeta` lets the UI test-fetch surface `link-header` pagination from response headers. `mode`/`state` and `triggerContext` (the resolved `{path}` → value map of a webhook-steered fetch, §10) are for the built-ins only; an app connector sees nothing but `query` and its own page `cursor`.

**Every fetch is a typed query** (`stream-query.ts`; design: `plans/data-connectors/v14/typed-query-brainstorm.md`). The platform decides *which records*; the connector honours the query exactly — nothing in it is a hint:

```ts
interface ConnectorQuery {
  ids?: string[]                          // exactly these records (refresh, re-import a selection, webhook steer)
  idKind?: string                         // with `ids` on a steer whose delivery carries a foreign id
  period?: { from?: string; to?: string } // UTC ISO on the stream's declared `period` path; from inclusive, to exclusive
  since?: unknown                         // the marker the app returned on the last page of the previous run
}
```

An app stream declares what it can be queried by — `query?: { ids?: true; period?: '<source path>'; since?: true }` on the catalog stream (`CatalogConnectorStream.query`). An absent key means unsupported. The builder per job:

| Job | Query |
| --- | --- |
| Backfill | `{ period: { from: floor } }` on a period stream when a floor exists, else `{}` |
| Steady run / sweep | a `since` stream: `{ period: { from: floor }?, since }` (`since` absent on the first steady run after a restart); any other stream: the backfill query again — a rescan stream re-reads every run |
| Re-import a period | `{ period: { from, to } }` |
| Refresh / re-import records | `{ ids }` |
| Webhook steer (app stream) | `{ ids: [value at idPath], idKind? }` |

The **floor** is `config.historyStartDate` at UTC midnight, moved *earlier* to an accounting-active org's cutover, never later (`historyFloor` in `slice-orchestrator.ts`; `min(date, cutoverStart)`). No date ⇒ no floor ⇒ everything. Generic REST gets the same floor through `query.period.from`, formatted into its `requestConfig.backfillWindow.sinceParam`. `syncMode` is derived: `catalogSyncMode(query)` returns `incremental` for a `since` stream and `snapshot` otherwise. A user-issued query the stream can't run is refused **before** the run (`missingQueryCapability` → `BadRequestError` naming the stream and the missing capability). A stream's stored `recordFilter` is never sent; the platform applies it post-fetch, and a period re-import re-checks the bounds on the declared `period` path the same way.
...
- **App connectors** (`app-connector-adapter.ts`) — declared in an app's SDK via `defineDataConnector` (`@auxx/sdk/data-connectors`). The adapter resolves the installed app's catalog connector and, inside `fetch()`, **lazy-imports** the app-runtime cluster (`@auxx/lib/apps`), resolves the borrowed connection (with lazy-refresh), and **drives a page-per-`execute` loop** against the sandbox via the lambda executor (`apps/lambda/src/executors/data-connector-executor.ts`). See §5 for the loop. Cursor translation between the engine's opaque `SyncCursor` and the app's flat cursor, and the `since` ↔ watermark encoding, live in `app-connector-state.ts`.
  ...
  - `ConnectorDeltaExpiredError` — the app threw the SDK's `DeltaExpiredError` (its `since` is stale: a `410 Gone` sync token, a pruned `historyId`). It crosses the sandbox as `deltaExpired: true`, like `rateLimited`, and the adapter re-throws it in-realm. `runBackfillSlice` clears the watermark, resets the stream to a fresh backfill (`restartExpiredDelta` → `freshBackfillState`) and re-enqueues inside the same run; a second expiry in the run fails it.
...
1. Decode the engine's `SyncCursor` to the app's flat page cursor (`decodeCursor`, in `app-connector-state.ts` — tolerant of malformed/legacy values, never fails a live sync).
2. Invoke the sandbox `execute({ streamKey, query, cursor?, connection, config })` via the lambda executor (records materialized, capped defensively at 5000/page). `query` is the same for every page of the run.
3. The app returns `{ records, cursor?, since?, rateLimited? }`, or throws `DeltaExpiredError`.
4. Yield the records, then emit a `ConnectorCheckpoint`: a page with a `cursor` carries `{ cursor: encodeCursor(cursor) }`; the page without one is terminal and carries `{ since: encodeSince(since) }`.
5. Repeat until terminal.

**Two cursors, two lifetimes.** `cursor` pages within one query and dies with it. `since` lives across runs: returned on the **last page only**, JSON-encoded into the stream's `watermark`, and handed back decoded as the next steady run's `query.since`. The slice loop replaces the watermark with it rather than folding it through `maxWatermark` — the platform never interprets an app's marker (a timestamp, a Gmail `historyId`, a Graph `deltaLink` all fit). A run that dies mid-way keeps the old `since`, so the next run repeats the delta rather than skipping part of it.

`encodeCursor`/`decodeCursor` keep the app's token opaque to the engine — always `kind: 'token'`, `value` a JSON string. The SDK contract (`define-data-connector.ts` JSDoc, `ConnectorExecuteArgs`/`ConnectorFetchResult` in `packages/sdk/src/root/data-connectors/types.ts`) tells authors: *"the platform loops `execute`, not the app — return ONE page plus `cursor`."* Cursor and `since` translation is unit-tested in `app-connector-state.test.ts`; the loop + resume-across-slices in `app-connector-adapter.test.ts`.
...
3. **Applies per-field merge strategy** and writes through `UnifiedCrudHandler.create/update` (which coerces every value through the typed FieldValueService converters). Option-bearing fields (`SINGLE_SELECT`/`MULTI_SELECT`/`TAGS`) go through `sinks/mint-option-keys.ts` first: a label the field lacks is minted via `mintOrMatchOptions` when `canGrowFieldOptions` ∧ `fieldAllowsNewOptions` allow it (system TAGS such as `order_payment_gateways` do; connector- and app-owned fields and system selects do not), so the stored `optionId` is a real option key rather than the raw text. Values written before this existed still carry the raw text as their key and resolve as unknown chips until the record is rewritten. Sink writes use **`skipEvents: true`** — the per-write fan-out (field hooks, activity/timeline, the BullMQ event bus, realtime) is suppressed to stay storm-proof on large syncs. To let automations still react to synced data, the sink accumulates a **sync-change manifest** (old→new per *subscribed* field, plus created/archived record ids) when the org has record rules watching those fields; at run finalize a single `sync:records:changed` event is published and consumed by the record-rules engine. This is the B2 mechanism — see `entity-events-architecture-guide.md` §8. (The CSV importer is the second manifest producer.) The same manifest also replays the registered **field-change hook chain** at finalize — marks on every touched key, derives only through a `batch` core — so derived fields stay current on synced data (`entity-events-architecture-guide.md` §7.2).
...
- **`SliceResult`** — what a slice reports: `{ recordsProcessed, pagesProcessed?, rateLimitWaitMs?, nextCursor?, hasMore, watermark?, commit, counters? }` (`nextCursor` is ignored when `commit === 'partial-retriable'`; `watermark` is opaque — the core stores it on advance and never compares it).
...
**2. Steering (per-stream) — how a matched delivery guides this stream's fetch.** Stored on `DataConnectorStream.requestConfig.webhookTrigger` (`StreamWebhookTrigger`). Both kinds share `filter` (topic discrimination via the same `matchesFilter` the agent/workflow app-trigger path uses — flagship apps multiplex many topics through one `triggerId`, e.g. Shopify's 22 topics on `triggerData.topic`) and `debounceMs`. Then they split:
- **App stream** — stamped verbatim from the catalog stream's `webhookTrigger: { filter?, idPath, idKind?, debounceMs? }`. The steer is a typed query: `{ ids: [value at idPath] }`, plus `idKind` when the delivery carries a foreign id (Shopify `inventory_levels/update` carries an inventory item id; the `product` stream declares `idKind: 'inventoryItem'` and resolves it in its own fetch). No `{path}` tokens reach the app.
- **Generic REST** — `paths` (payload paths exposed as `{path}` placeholders), `deleteWhen` + `deleteExternalIdPath` (delete predicate + id to archive), `resultShape` (`single`/`collection`). Edited in `webhook-steering-section.tsx`.

The principle: **the webhook is the signal; the fetch is the truth** — a delivery never gets sunk raw; it steers the canonical fetch.
...
3. The child runs **`runWebhookEventSlice`** (`connector-webhook.ts`): `resolveWebhookSteer(webhookTrigger, triggerData)` returns `delete`, `ids` or `fetch`. A **delete** archives by `deleteExternalIdPath` (`archiveExternalId`). An **`ids`** steer (app stream) runs the **normal `definition.fetch`** with `query: { ids, idKind? }`; a **`fetch`** steer (generic REST) runs it with `query: {}` and `triggerContext` (the resolved `{path}` values). Either way it is identical auth/baseUrl/pagination/mappings to a bulk sync, and the result sinks through the **same `sinkSourceRecord` → entity-sink path**. A delivery whose `idPath` doesn't resolve is not steerable (`isSteerableDelivery`) and falls through to a full sync.

**A webhook is a POINT WRITE, not a run.** `runWebhookEventSlice` opens **no `DataConnectorRun`** (synthetic `runId = app-webhook:<eventId>`), never stamps `lastSeenRunId` or reads or writes the watermark/`since`/cursor (that would skew orphan reconciliation and the steady delta), and stamps `lastWebhookEventAt` (`stampWebhookEvent`) for liveness — the only "synced" signal a pure-webhook connector has. Idempotent via the receiver's event-id dedup + the sink's content-hash skip. A throttle surfaces as `ConnectorRateLimitError` (fetch sets `maxRetries: 0`); the child job re-enqueues with the provider `Retry-After` and dead-letters after exhausting retries. Steer resolution is unit-tested in `webhook-steer.test.ts`; dispatch in `{app-trigger,webhook-endpoint}-sync-dispatch-job.test.ts`.

`dataConnectorSyncBehavior = 'webhook'` still selects webhook-driven sync (it gates which connectors a dispatch even considers). UI: `webhook-signal-section.tsx` (connector-level signal picker — app trigger vs endpoint; an app connector's signal and steering render read-only, summarised by `lib/describe-steering.ts`) + `webhook-steering-section.tsx` (generic-REST per-stream topic/path/delete steering) — the old signing-secret/URL gap is closed.
...
- **Scheduler:** `data-connector-scheduler.ts` — `reconcileConnectorSchedulers` / `syncConnectorScheduler` / `removeConnectorScheduler`, driven by the connector's `ScheduledTriggerConfig` (shared agent/workflow frequency model). Plus `syncConnectorSweepScheduler` — the periodic self-heal (`trigger: 'sweep'`) for missed webhook deliveries. It sends the steady run's query: a `since` stream does its catch-up, a floorless snapshot stream re-crawls with `{}` and reconciles, a floored stream re-reads its period and does not.
...
**Orphan behavior** (`OrphanBehavior`) — REVISED v12. Crawl reconciliation is the **self-healing** delete channel: a webhook that was never delivered is never redelivered, but every crawl re-answers the question. `reconcileOrphans` (`reconciliation.ts`) acts on an item absent from the run when three gates pass, and each means something different:

- **A snapshot stream fetched with an unbounded `{}` query** (`streamReconcilesByAbsence(stream, query)`, `isUnboundedQuery`) — absence only means deletion when the fetch asked for everything. A `since` stream sees a delta, a floored stream sees a period, an `ids` query sees a selection: absence there means "not asked for", so none of them reconcile. Setting `historyStartDate` on a period stream therefore turns its delete-by-absence off. Not negotiable; the deferred id-only reconcile crawl is what relaxes it correctly.
- **`linkMode === 'upsert'`** — a reference mapping writes nothing, so it owns nothing to archive.
- **`orphanBehavior !== 'ignore'`** — **the mapping must ASK.** This replaced the old owned-only rule: an explicit per-mapping declaration is the consent that check stood in for, and it lets a *contributing* mapping (Shopify products are contributing) opt in without opening every co-owned record to archival. Apps declare it on the mapping (`ConnectorMappingBase.orphanBehavior`); the default is still `'ignore'` in all four defaulting sites.

Two safety rules sit on top, because a **completed** crawl can still be wrong (a filtered query, a narrowed auth scope, an empty page reported as done — none of which trip `finalizeBackfill`'s completeness gating):

- `effectiveOrphanBehavior` refuses to **archive** a record this connector did not mint (`DataConnectorItem.mintedInstance`, the same flag `deleteConnector` uses). Enriching someone else's contact is not authority to remove it; that degrades to `mark_deleted`.
- `ARCHIVE_CAP` / `archiveCapReason` refuses the **whole pass** when the orphan set is implausible (a total wipe at any size, >500 rows, or >20% once the set is ≥25). It archives nothing, pushes an untiered `errorSample` entry so the run reports `partial`, and waits for a human. This is the difference between "a product was deleted" and "the crawl broke and we archived the catalog".

**Per-parent child sets** (`child-sets.ts`). A fan-out child mapping (`parentMappingId` set, `upsert`, a `[]` in its `rootPath`) that declares an orphan behavior is also reconciled **per parent, on every lane** — backfill, delta and webhook — because a parent's payload is complete for its own children even when a crawl is not complete for the whole set. `mapRecordTree` emits a `ChildSet` per parent whose array was **present** (a missing key emits nothing; `[]` means "no children"); after the record's writes, `replaceChildSets` stamps `DataConnectorItem.parentExternalId` on the present children and applies `effectiveOrphanBehavior` to that parent's other live children. No cap applies (the blast radius is one parent), and it skips a payload whose root the out-of-order guard judged stale. Singletons (`customer`) are excluded: one embedded record shared by many parents belongs to none of them. The motivating case is Shopify `tax_lines[]`, whose synthetic `order:title:rate` key changes whenever a rate or title does. Rows bound before the column existed are stamped by `packages/lib/scripts/backfill-connector-item-parents.ts`.

`mark_deleted` is real as of v12 (it was a no-op stub): it stamps `DataConnectorItem.removedUpstreamAt`, bumps the run's `markedDeleted` counter, and leaves the record fully live. `upsertItem` clears the stamp when the record reappears, so it self-heals. Explicit delete signals (a `deleted` tombstone, a webhook delete) flow through `archiveExternalId` instead and are subject to **neither** safety rule — the upstream said so outright, which is stronger than absence. `handleConnectorDelete` remains **uncalled** dead code, and `resolveDelete` is implemented only by `connectors/fixture.ts`.
...
- **Connector CRUD (`adminProcedure`):** `create` (incl. `createConnectorFromAppCatalog` / `createConnectorFromTemplate`), `update`, `delete` (with keep/archive/delete-synced-records behavior), **`syncNow`** (enqueue a manual sync), **`reimport`** (`{ streamIds, query: { ids } | { period: { from?, to? } } }` — validated by `validateReimportQuery`, refused pre-run when a stream lacks the capability), **`refreshRecord`** (the record drawer's refresh — an `{ ids: [externalId] }` re-import on the record's root stream), **`provision`** (provision target schema on demand).
...
- **SDK:** `packages/sdk/src/root/data-connectors/` (`defineDataConnector`), exported at `@auxx/sdk/data-connectors`. App catalogs serialize `app.dataConnectors` → `AppDeployment.catalog.dataConnectors`, projected onto the `installedApps` org-cache key for bundle-free discovery. The SDK page-per-`execute` contract lives in its JSDoc + `ConnectorQuery`/`ConnectorStreamQueryDecl`/`ConnectorExecuteArgs`/`ConnectorFetchResult` and `DeltaExpiredError`.
...
**Webhook-driven:** with `syncBehavior: 'webhook'`, the connector binds a **signal** (`config.webhookTrigger` — an app trigger or a generic `WebhookEndpoint`) and each stream declares **steering** (`requestConfig.webhookTrigger` — topic filter + `{path}` tokens + delete rule). A verified delivery → `dispatch*ToConnectors` (match by signal + per-stream `filter`) → `app-trigger-sync-stream` child → `runWebhookEventSlice`: the delivery steers the **normal fetch** (an app stream's `{ ids }` query, or generic REST's `{path}` tokens), and the fetch result rides the same mapping → sink path (near-real-time, no poll, no run — a point write). A delete event archives by externalId instead of fetching.

⚠️ **The webhook is the latency path, not the source of truth (v12).** A push signal is at-least-once *at best*: Shopify retries a delivery for 48 hours and then drops it for good, and no delivery arrives at all if the app was reinstalled, the connector was paused, the worker was down past the retry window, or the record was deleted before the connector existed. One missed delete means that record is wrong forever, because nothing re-asks. The **crawl** is what makes deletes correct (see *Orphan behavior* above); an app connector cannot even declare `deleteWhen` (the SDK's per-stream `webhookTrigger` has only `filter`/`idPath`/`idKind`/`debounceMs`, and the steering editor is gated on generic-REST), so for app connectors the crawl is the *only* delete channel.
...
- **Reconciliation gates on backfill completion** (`finalizeBackfill`, last stream only via the latch) — a partial backfill must never archive unreached records.
- **Only an unbounded `{}` reconciles.** A stream deletes by absence only when it is a snapshot stream whose run query had no `ids`, no `period` and no `since` (`streamReconcilesByAbsence(stream, query)`). The query is built in one place (`syncQuery`, `stream-query.ts`) and the reconcile gate reads the same builder, so the two can't disagree. A stored `recordFilter` is applied post-fetch and never narrows the query. See `plans/data-connectors/v14/implementation-brief.md` §3.
- **An app's `since` is opaque and replaced, never compared.** It is written on the terminal checkpoint only, and a re-import, refresh or webhook steer never reads or writes it.
- **A re-import never touches the stream's durable state (v13 N5).** `startConnectorSync({ reimport })` (entry: `requestReimport` in `reimport.ts`, router `dataConnector.reimport`) opens a `mode: 'reimport'` run over the named streams with the user's query on `DataConnectorRun.query`; each slice pages on `DataConnectorRun.progress.cursors.<streamId>` (`createRunSyncStateStore`), the fetch sends that query verbatim in place of the sync query (never `since`, never the floor), and finalize runs without `reconcileOrphans`/`reconcileManagedMarkers` or `clearResyncPending`. Refresh and re-import are app-connector only; generic REST can't be queried by id or period and is refused. An `ids` run and a parked run's continuation carry `retryClaim`: a claimed connector throws `ConnectorClaimedError` and BullMQ retries the job (15s × 40); a period run that loses the race is dropped with a log, and a backfill continuation no-ops once a newer sync run resumed the park (`isNewestSyncRun`).
...
- **A `disconnected` connector is structurally indistinguishable from a healthy one — gate every sync door on `status`, never on config completeness.** Uninstalling an app sets its connectors to `'disconnected'` (`disconnectConnectors`) but deliberately **preserves the credential**, so a disconnected app connector still has a bound connection and fully-configured streams: every config-shaped predicate says yes. There are **five** doors onto a sync and they are guarded in two different places — the scheduler and the two webhook dispatch jobs filter `SUSPENDED_CONNECTOR_STATUSES` (`['paused', 'disconnected']`), while the three *manual* doors (`syncNow`, `sampleFetch`, `backfillPendingChange`) gate on `getConnectorReadiness`, which is why that predicate takes `status` and refuses `disconnected`/`deleting` **before** any config check. `claimForSync` is not a backstop: it only refuses a connector already `'syncing'`. A new manual door that skips the readiness predicate reopens this.
- **`status = 'disconnected'` is load-bearing state, not a label.** `reconnectConnectorsForInstallation` repairs connectors on reinstall with `eq(status, 'disconnected')` — scoped deliberately, so a connector the merchant paused *themselves* before the uninstall is not silently re-labelled. The consequence is that **anything which moves a connector off that status between uninstall and reinstall removes it from the repair path permanently**: it lands in `error`, the "the app was removed, reinstall it" banner is replaced by a generic failure, and reinstalling no longer fixes it. `paused` is deliberately still allowed to sync manually — the merchant can resume it themselves; `disconnected` they cannot.
- **Uninstall keeps the records AND their contents.** `EntityInstance` stores no user data — every value lives in `FieldValue` — so "keep the synced records" is only true if the app's `CustomField` rows survive with them. The app-manifest sweep is therefore deferred: an installation's columns are removed when it is uninstalled **AND** has no surviving `DataConnector`, never on uninstall alone. Deleting a connector while the app is still installed must **not** sweep (the Shopify chat proxy writes `customerId` through app → installation → connection, with no connector in the chain).
