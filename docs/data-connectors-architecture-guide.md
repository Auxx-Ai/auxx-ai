...
| `DataConnectorStream` | `data-connector-stream.ts` | One fetch = one source schema. Holds `streamKey` (nullable — a draft is created blank and named inline), `requestConfig` (jsonb), `sourceSchema` + `schemaSource` (`catalog`/`inferred`/`manual`), `syncMode` (`snapshot`/`incremental`/`webhook`), `recordFilter` (jsonb condition groups over source paths, evaluated per raw record by `record-filter.ts`; an app seeds it from the stream's `recordFilter` declaration and a redeploy updates it like `syncMode`), and **`state`** (jsonb `ConnectorStreamState` — the durable per-stream sync state: phase, backfill cursor, watermark, recordsSeen). |
...
- **`syncMode === 'snapshot'`** — absence only means deletion when the fetch saw everything. An incremental stream sees a delta, so absence there means "unchanged". Not negotiable; the deferred id-only reconcile crawl is what relaxes it correctly.
...
- **Connector CRUD (`adminProcedure`):** `create` (incl. `createConnectorFromAppCatalog` / `createConnectorFromTemplate`), `update`, `delete` (with keep/archive/delete-synced-records behavior), **`syncNow`** (enqueue a manual sync), **`reimport`** (a period or `$externalId in` re-import, v13 N5), **`provision`** (provision target schema on demand).
...
- **A narrowed fetch never deletes by absence (v13 N4).** App streams are sent `recordFilter` clauses built by `sentClauses` (`connector-sync-source.ts`): the AND spine of the stored filter (`pushableClauses`) plus the `exact` backfill floor on a stream with a catalog `periodField`. A stream sent any clause is excluded from `reconcileOrphans`, and a stream that could reconcile is never sent its stored filter. See `plans/data-connectors/v13/narrowed-fetch-plan.md`.
- **A re-import never touches the stream's durable state (v13 N5).** `startConnectorSync({ reimport })` (entry: `requestReimport` in `reimport.ts`, router `dataConnector.reimport`) opens a `mode: 'reimport'` run over the named streams; each slice pages on `DataConnectorRun.progress.cursors.<streamId>` (`createRunSyncStateStore`), the fetch is forced to `snapshot` so the app gets no `updatedSince`, the run clauses (`run.recordFilter`) go `exact` in place of the floor, and finalize runs without `reconcileOrphans`/`reconcileManagedMarkers` or `clearResyncPending`. An `id` run and a parked run's continuation carry `retryClaim`: a claimed connector throws `ConnectorClaimedError` and BullMQ retries the job (15s × 40); a period run that loses the race is dropped with a log, and a backfill continuation no-ops once a newer sync run resumed the park (`isNewestSyncRun`).
