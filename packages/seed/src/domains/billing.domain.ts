...
interface PlanDefinition {
  ...
  /**
   * Shopify App Pricing plan handle. Generated by Shopify in the Partner Dashboard
   * (manual, one-time per environment — see plans/billing/v2/03-schema-and-plan-mapping.md
   * §2.1) and mirrored here. Handles match across dev/prod. `null` = not billable via
   * Shopify (Enterprise = sales-led, Demo = internal).
   */
...
const STATIC_LIMITS = {
  ...
  free: {
    ...
    workflowsLimit: 3,
    sequencesLimit: 0,
    // Deliberately NOT 0. Filters carry no marginal cost (unlike sequences,
    // datasets and agents, which are 0 here) and are the clearest "better than
    // plain Gmail" moment — and `0` reads as *feature off* in this schema,
    // which is a stronger statement than intended.
    mailFiltersLimit: 5,
    savedViews: 10,
    knowledgeBases: 1,
