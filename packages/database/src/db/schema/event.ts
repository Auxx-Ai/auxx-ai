...
export const Event = pgTable(
  ...
  (table) => [
    ...
    // Targeted expression index for the chat-widget history read path
    // (P4.4 hydration), which queries `data->>'threadId'` for thread
    // lifecycle events.
    index('Event_threadId_expr_idx').on(sql`((data->>'threadId'))`),
  ]
)
