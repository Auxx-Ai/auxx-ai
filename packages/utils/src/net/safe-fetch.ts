// packages/utils/src/net/safe-fetch.ts
...
export class BlockedAddressError extends Error {
  ...
  constructor(readonly address: string) {
    super(`Refusing to connect to a private or reserved address (${address})`)
    this.name = 'BlockedAddressError'
  }
}

/** A URL `safeFetch` refuses before any network call (unparseable or not http/https). */
...
function parseHttpUrl(url: string | URL): URL {
  ...
  try {
    ...
    throw new UnsafeUrlError(`Invalid URL: ${String(url)}`)
  }
...
/** npm undici's fetch sends the global FormData as "[object FormData]", so encode it to multipart bytes. */
async function encodeFormData(init: RequestInit): Promise<RequestInit> {
...
export async function safeFetch(url: string | URL, init: SafeFetchInit = {}): Promise<Response> {
  ...
  try {
    const response = await undiciFetch(parsed, {
      ...((await encodeFormData(rest)) as Parameters<typeof undiciFetch>[1]),
