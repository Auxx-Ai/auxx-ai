// packages/lib/src/postings/__tests__/post-inventory-movement.test.ts
//
...
vi.mock('../accounting-enabled', () => ({
  isAccountingEnabled: async () => h.accountingEnabled,
}))
vi.mock('../period-lock', () => ({ resolvePeriodLock: async () => ({ lockedThrough: null }) }))
vi.mock('../post-entry', () => ({
