...
import { accountingBasisHash, fulfillmentAccountingEffectKey } from './effect-basis'
import { type AccountingWorkBasisInput, accountingWorkBasisSchema } from './effect-types'
...
export async function captureFulfillmentWorkInTx(
  ...
  const basis = accountingWorkBasisSchema.parse(input.basis)
