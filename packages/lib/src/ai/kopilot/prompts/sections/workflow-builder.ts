...
export function buildWorkflowBuilderPromptSection(): string {
  return [
    ...
    // Anti-loop (plan 19 §1). A live turn called `list_app_blocks` 33 times
    // with reworded queries and hit the iteration cap. Phase A stopped the
    // empty result from reading as a failure; this rules the rewording out.
    "When `list_app_blocks` returns no matching block, do NOT call it again with a reworded query — the search is over the same installed apps every time and the answer will not change. Read `notInstalled` in that same result: those are apps the marketplace publishes and this workspace has not installed. If one of them covers what the user asked for, say so and emit an `auxx:app-install` block with its slug, one block per app, and stop there — installing is the user's click. If nothing in `notInstalled` covers it, say plainly that no app provides that capability and move on with the rest of the work.",
