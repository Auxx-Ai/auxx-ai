    ...
    items: {
      ...
      properties: {
        ...
        operator: {
          type: 'string',
          description:
            'Filter operator. Common: "is", "is not", "contains", ">", "<", ">=", "<=", "empty", "not empty", "in", "not in". Dates: "within_days" / "older_than_days" (value = number of days), "before" / "after" / "on_date" (value = absolute date "YYYY-MM-DD"), "between" (value = { "from": "YYYY-MM-DD", "to": "YYYY-MM-DD" }, to exclusive, either end optional), "today" / "yesterday" / "this_week" / "this_month" (no value).',
        },
        value: {
          description:
            'Comparison value. Use the option value key for select fields (e.g. "ACTIVE" not "Active"). For "in"/"not in", pass an array. For dates, "in the last N days" is "within_days" with a NUMBER (e.g. 30) — NOT "after" with a relative string like "now-30d"; "before"/"after"/"on_date" take an absolute "YYYY-MM-DD" date; "between" takes { "from", "to" } with "to" exclusive (August is { "from": "2026-08-01", "to": "2026-09-01" }).',
        },
      },
