// packages/lib/src/field-values/insert-value.ts

import { database, schema } from '@auxx/database'
import { fromDatabase } from '@auxx/services/shared/utils'
import { parseRecordId } from '@auxx/types/resource'
import { ok } from 'neverthrow'
import type { InsertFieldValueInput, ServiceFieldValueRow } from './service-types'
...
export async function insertFieldValue(input: InsertFieldValueInput) {
  ...
  return ok(dbResult.value[0] as ServiceFieldValueRow)
}
...
export async function batchInsertFieldValues(inputs: InsertFieldValueInput[]) {
  ...
  return ok(dbResult.value as ServiceFieldValueRow[])
}
