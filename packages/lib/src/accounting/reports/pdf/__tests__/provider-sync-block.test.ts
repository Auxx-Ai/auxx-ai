// packages/lib/src/accounting/reports/pdf/__tests__/provider-sync-block.test.ts
//
...
import type { ProviderSyncMarker, ProviderSyncReading } from '../../../mirror/client'

type Client = typeof import('../../../mirror/client')
...
vi.mock('../../../mirror/client', async (importOriginal) => {
  const actual = await importOriginal<Client>()
  return { ...actual, describeProviderSyncCoverage: describeCoverage }
})

const { describeProviderSyncCoverage } = await vi.importActual<Client>('../../../mirror/client')
