// packages/lib/src/accounting/sales/quotes/convert-quote.ts
 ...
 *
 * MIGRATION follow-up 7: writes `MoneyTransaction.workOrderInstanceId` directly,
 * never `MoneyCommand.actorSnapshot`.
 */
export async function stampQuoteDepositsOnWorkOrder(params: {
  ...
  await database
    ...
    .set({ workOrderInstanceId: params.workOrderInstanceId })
    .where(
      and(
        eq(schema.MoneyTransaction.organizationId, params.organizationId),
        eq(schema.MoneyTransaction.quoteInstanceId, params.quoteInstanceId)
      )
    )
}
