...
export interface ChartAccountRow {
   ...
   *
   * 🛑 Stamped by the `chartAccounts` cache provider. Only `listChartAccounts`
   * and `loadChartAccountsById` may filter on it - every other reader gets live
   * rows only, which is what makes archiving removal as far as the resolver, the
   * role picker and the close are concerned (`removeChartAccount`'s reason 1).
   */
  isArchived?: boolean
}
