...
/**
 * Notify a visit's assignee that its time changed (old → new) — every user the assignee worker
 * resolves to (a team notifies ALL members, 45-teams.md §5.4). No-op when the visit has never
...
export async function notifyVisitRescheduled(input: NotifyVisitRescheduledInput): Promise<void> {
  ...
  if (!visit.dispatchedAt || !visit.assigneeWorkerId || !visit.startTime || !visit.endTime) return
  const recipientUserIds = await resolveWorkerUserIds(organizationId, visit.assigneeWorkerId)
  ...
  try {
    ...
    for (const recipientUserId of recipientUserIds) {
      ...
      if (await emailPrefEnabled(organizationId, recipientUserId)) {
        const assignee = await findUserById(recipientUserId)
        if (assignee?.email) {
...
/**
 * Notify a visit's assignee that it was canceled/unscheduled — every user the assignee worker
 * resolves to (a team notifies ALL members, 45-teams.md §5.4). No-op when the visit has never
...
export interface NotifyVisitReassignedInput {
   ...
   * current schedule). */
  visit: WorkOrderVisitRow
  /** Assignee worker before the write; `null` if it was previously unassigned. */
  oldAssigneeWorkerId: string | null
}

/**
 * Notify BOTH sides of a reassignment: a "removed" notice to every user the OLD assignee
 * worker resolves to (if any) and an "assigned" notice to every user the NEW one resolves to
 * (if any) — a team assignment notifies ALL its members (45-teams.md §5.2/§5.4). Diffed at
 * the resolved-user level, so a reassignment between two workers that share a member (e.g. a
 * team → one of its own members) doesn't double-notify that user — never the full dispatch
 * email (Dispatch stays the only sender of that). No-op entirely when the visit has never been
 * dispatched.
 */
export async function notifyVisitReassigned(input: NotifyVisitReassignedInput): Promise<void> {
  const { organizationId, userId, visit, oldAssigneeWorkerId } = input
  if (!visit.dispatchedAt || !visit.startTime || !visit.endTime) return
  const newAssigneeWorkerId = visit.assigneeWorkerId
  if (oldAssigneeWorkerId === newAssigneeWorkerId) return

  const [oldUserIds, newUserIds] = await Promise.all([
    oldAssigneeWorkerId
      ? resolveWorkerUserIds(organizationId, oldAssigneeWorkerId)
      : ([] as string[]),
    newAssigneeWorkerId
      ? resolveWorkerUserIds(organizationId, newAssigneeWorkerId)
      : ([] as string[]),
  ])
  ...
  if (removedUserIds.length === 0 && addedUserIds.length === 0) return
  ...
  for (const targetUserId of addedUserIds) {
    await notifySide(
      targetUserId,
      'assigned',
      'VISIT_REASSIGNED' as NotificationType,
