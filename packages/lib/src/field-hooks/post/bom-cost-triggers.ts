...
/** Recalculate a known set of parts; for handlers that resolve parts another way (tariff rates). */
export async function recalculatePartCostsForParts(
  organizationId: string,
  partIds: readonly string[]
): Promise<void> {
  if (partIds.length === 0) return
  await recalculateAffectedParts(organizationId, [...new Set(partIds)])
}
