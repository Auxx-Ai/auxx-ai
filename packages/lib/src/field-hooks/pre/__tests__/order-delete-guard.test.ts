// packages/lib/src/field-hooks/pre/__tests__/order-delete-guard.test.ts
...
// judged on its stock movements, an order on the `GlPostingSource` rows that
// name it (TARGET §1's one-query promise), and those carry a calendar
// `txnDate` rather than a timestamp, which is what the timezone case below
// pins.
...
// terminal `.where()` resolves it, so a query that stops ending there fails
// loudly.
...
describe('guardOrderDelete: refusal', () => {
  ...
  it('falls back to the posting id when the entry has no document number', async () => {
    postings(posting('2026-08-15', 'posted', null))

    await expect(guardOrderDelete(event())).rejects.toThrow(/gl-2026-08-15-posted/)
  })

  // Decision 9: reversing a posting is what makes its order deletable again,
  // and is also what frees the fulfillment's claim to post again.
  it('allows a delete once every posting is reversed', async () => {
    postings(posting('2026-08-15', 'reversed'))
...
describe('guardOrderDelete: the calendar date', () => {
  ...
  it('does not drag the first of an open month back into a locked one', async () => {
    postings(posting('2026-08-01', 'reversed'))
    h.resolvePeriodLock.mockResolvedValue({ lockedThroughMonth: '2026-07' })
...
describe('guardOrderDelete: open books', () => {
  it('passes when every entry is reversed and none is settled', async () => {
    postings(posting('2026-08-15', 'reversed'), posting('2026-08-16', 'reversed'))
  ...
  it('skips every read entirely for an order with no postings at all', async () => {
  ...
  it('looks the entries up by the order source, keyed on the order instance', async () => {
    await guardOrderDelete(event())
