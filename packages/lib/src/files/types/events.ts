 ...
 */
export type EventHandler<T extends FileUploadEvent = FileUploadEvent> = (event: T) => void

/**
 * Event handlers interface
 */
export interface EventHandlers {
  onUploadProgress?: EventHandler<UploadProgressEvent>
  onProcessingProgress?: EventHandler<ProcessingProgressEvent>
  onUploadCompleted?: EventHandler<UploadCompletedEvent>
  onProcessingCompleted?: EventHandler<ProcessingCompletedEvent>
  onJobUpdate?: EventHandler<JobUpdateEvent>
  onError?: EventHandler<ErrorEvent>
  onSessionConnected?: EventHandler<SessionConnectedEvent>
  onAnyEvent?: EventHandler<FileUploadEvent>
}

/**
 * Channel naming utilities (pure functions)
 */
export const FileUploadChannels = {
  /**
   * Primary channel for upload session events
   */
  session: (sessionId: string): string => `file:upload:${sessionId}`,

  /**
   * Entity-specific channel for processing events
   */
  entity: (entityType: string, entityId: string): string => `file:entity:${entityType}:${entityId}`,

  /**
   * Organization-scoped channel for batch operations
   */
  organization: (organizationId: string): string => `file:org:${organizationId}`,

  /**
   * Job-specific channel for background job updates
   */
  job: (jobId: string): string => `file:job:${jobId}`,
} as const

/**
 * Event validation utilities (pure functions)
 */
export const FileUploadEventValidator = {
  /**
   * Validate event has required fields
   */
  isValidEvent: (event: any): event is FileUploadEvent => {
    return (
      event &&
   ...
   */
  isEventOfType: <T extends FileUploadEventType>(
