 ...
 * - **Instance-access resources** (`dataset`, `kb`, `dashboard`) — governed by
 *   their own L2 area + per-instance `ResourceAccess` grants (the Share card),
 *   disjoint from type-level def enforcement (plan 08 §0.6 / 11 §0.6 / 12 §0.11
 *   / 13 §0.6). `article` inherits its KB's grants (no per-article grants). A
 *   def-access grid row for any of these would be a phantom control that writes
 *   rows nothing reads.
 *
