// packages/lib/src/data-connectors/__tests__/managed-fields.test.ts
//
...
// is an array of raw target-ref strings shaped `<defId>:<fieldId>` (never a bare attribute),
// the caller passes the concrete field id, archived items never count, and a record bound to more than one live item is answered by
...
describe('isFieldConnectorManaged', () => {
  it('returns true when a live item lists the field', async () => {
    const db = fakeDb([{ managedFields: ['def_o:f-o-total', 'def_o:f-o-subtotal'] }])
    expect(await isFieldConnectorManaged(db, 'org_1', 'inst_1', 'f-o-total')).toBe(true)
  })

  it('returns false when no item lists the field', async () => {
    const db = fakeDb([{ managedFields: ['def_o:f-o-subtotal'] }])
    expect(await isFieldConnectorManaged(db, 'org_1', 'inst_1', 'f-o-total')).toBe(false)
  })
  ...
  it('answers true if ANY of several live items names the field, not just the first', async () => {
    const db = fakeDb([{ managedFields: [] }, { managedFields: ['def_o:f-o-total'] }])
    expect(await isFieldConnectorManaged(db, 'org_1', 'inst_1', 'f-o-total')).toBe(true)
  })
  ...
  it('matches the line-total attribute the same way it matches a document total', async () => {
    const db = fakeDb([{ managedFields: ['def_li:f-li-total'] }])
    expect(await isFieldConnectorManaged(db, 'org_1', 'line_1', 'f-li-total')).toBe(true)
  })

  it('does not match an attribute name against a defId:fieldId ref', async () => {
    const db = fakeDb([{ managedFields: ['def_o:f-o-total'] }])
    expect(await isFieldConnectorManaged(db, 'org_1', 'inst_1', 'order_total')).toBe(false)
  })

  it('accepts a bare field id ref and refuses an empty field id', async () => {
    const db = fakeDb([{ managedFields: ['f-o-total'] }])
    expect(await isFieldConnectorManaged(db, 'org_1', 'inst_1', 'f-o-total')).toBe(true)
    expect(await isFieldConnectorManaged(db, 'org_1', 'inst_1', '')).toBe(false)
  })
})
