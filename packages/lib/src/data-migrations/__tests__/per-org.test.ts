// packages/lib/src/data-migrations/__tests__/per-org.test.ts
...
function migrationThatThrows(thrown: (organizationId: string) => unknown): PerOrgMigration {
  return {
...
async function runAndCatch(db: Database, migration: PerOrgMigration): Promise<Error> {
  try {
    await perOrgMigration(migration).run(db)
  } catch (error) {
    return error as Error
  }
  throw new Error('expected the per-org migration to throw')
}

describe('perOrgMigration failure reporting', () => {
  /**
   * The whole point of the fix: `perOrgMigration` hands this aggregate straight
   ...
   * nothing for any of the ~97 wrapped per-org migrations.
   */
  ...
  it('does not throw when every org succeeds', async () => {
    const migration: PerOrgMigration = {
      id: '069-thread-search-text',
      description: 'test migration',
      up: async () => ({
        entityDefsCreated: 0,
    ...
    await expect(
      perOrgMigration(migration).run(dbWithOrgs(['org_1', 'org_2']))
