// packages/lib/src/kb/markdown/embed.ts
//
// Function-only mirror of `packages/ui/src/components/kb/utils/embed.ts`.
// Lives here so the server-side renderer (which sits in `@auxx/lib`) can
// resolve embed URLs without depending on `@auxx/ui`.
...
/**
 * Map a user-pasted URL to a sandboxed iframe `src` for one of the supported
 * providers. Returns `null` for anything else — raw user iframes are never
 * accepted.
 */
