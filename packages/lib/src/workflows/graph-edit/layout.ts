// packages/lib/src/workflows/graph-edit/layout.ts

// Ported verbatim from apps/web/src/components/workflow/utils/layout-algorithms.ts
// (Phase 3 §4) so a single dagre algorithm positions both agent-added and
// canvas-added nodes. Pure TS, no DOM/React — the React Flow `FlowNode` /
// `FlowEdge` params became the structural `LayoutNode` / `LayoutEdge` below
// (web's types are assignable to them); the position math is unchanged.
...
/** Structural subset of a React Flow node that layout actually reads. */
...
/** Structural subset of a React Flow edge that layout actually reads. */
...
export function getLayoutByDagre(
  originNodes: LayoutNode[],
  originEdges: LayoutEdge[]
