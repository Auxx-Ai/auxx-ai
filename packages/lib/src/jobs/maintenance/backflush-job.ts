...
import { endOfLocalDay } from '../../inventory/builds/backfill-builds'
...
export interface BackflushJobData {
  ...
  /** ISO instants; the range is read as local days in the org's book time zone. */
  from: string
  to: string
  actorUserId?: string
}
...
export async function backflushJob(ctx: JobContext<BackflushJobData | undefined>): Promise<void> {
  ...
  for (const organizationId of organizations) {
    ...
    try {
      ...
      const yesterday = endOfLocalDay(yesterdayInZone(now, timeZone), timeZone)
      const result = await backflushBuilds(database, organizationId, {
        from: yesterday,
        to: yesterday,
        now,
