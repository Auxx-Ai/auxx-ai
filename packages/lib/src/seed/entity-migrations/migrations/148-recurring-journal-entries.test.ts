// packages/lib/src/seed/entity-migrations/migrations/148-recurring-journal-entries.test.ts
//
...
describe('migration 148 registration', () => {
  ...
  it('is the only migration claiming the number 148 in the shared space', () => {
    const claiming = ALL_ENTITY_MIGRATIONS.map((m) => m.id).filter((id) => id.startsWith('148-'))
