// packages/lib/src/seed/entity-migrations/migrations/150-strip-legacy-address-components.test.ts
//
...
//  - registration in `ALL_ENTITY_MIGRATIONS`.
...
const { WORK_ORDER_FIELDS } = await import(
  '../../../resources/registry/resources/work-order-fields'
)
...
describe('migration 150 registration', () => {
  it('is registered in ALL_ENTITY_MIGRATIONS', () => {
    expect(ALL_ENTITY_MIGRATIONS.map((m) => m.id)).toContain(MIGRATION_ID)
  })
