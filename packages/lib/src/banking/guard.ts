// packages/lib/src/banking/guard.ts
 ...
 *
 * Copied from `money/bank-deposits/guard.ts` per `docs/lib-module-guide.md` §3 -
 * small enough that duplicating it beats a shared abstraction, and duplicating
 * it is what lets this module bind its own logger scope, so a refused bank
 * account write is greppable as `scope='banking'`.
 */
