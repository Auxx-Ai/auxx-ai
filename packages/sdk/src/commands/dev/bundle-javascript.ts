...
export function bundleJavaScript(
  ...
  async function handleBuild(): Promise<Fetchable<void, BuildContextError>> {
    ...
    try {
      ...
      // Compile and extract surface catalog (plans/kopilot/agents/triggers/app-surface-implementation-plan.md §5.3).
      ...
      if (isErrored(catalogResult)) {
        ...
        process.stderr.write(
          chalk.yellow(
            `⚠ Catalog extraction failed (${err.code}) — deployment will ship with no catalog${detail}\n`
          )
        )
      } else {
