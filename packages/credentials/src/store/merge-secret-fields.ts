...
/**
 * Partially update the nested `secrets.fields` bag (multi-field connection variables).
 * Multi-field secrets live under `secrets.fields[key]` (see `saveConnection`'s `pickSecrets`),
 * so a flat `mergeSecrets` would write to the wrong place; this merges into `fields` instead.
 *
 * `undefined` or `''` values in `partial` keep the existing field value (the "blank secret
 * field keeps the old value" edit semantics). Sibling secret keys (`accessToken`,
 * `refreshToken`, `secret`) are left untouched. Org-scoped.
 */
export async function mergeSecretFields(
  id: string,
