...
export async function updateVendorPart(input: UpdateVendorPartInput) {
  ...
  const result = await fromDatabase(
    database
      ...
      .where(
        and(eq(schema.VendorPart.id, id), eq(schema.VendorPart.organizationId, organizationId))
      )
      .returning(),
    'update-vendor-part'
  )
 ...
 */
export async function clearOtherPreferred(
  input: { partId: string; excludeId: string } & VendorPartContext
) {
